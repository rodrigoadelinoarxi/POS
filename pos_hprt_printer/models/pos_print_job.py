...
class PosPrintJob(models.Model):
    ...
    def _build_receipt_payload(self, order, frontend):
        ...
        # O frontend envia o objeto completo usado pelo próprio ecrã de recibo
        # do Odoo. Mantemo-lo intacto para preservar campos de módulos fiscais,
        # textos, unidades, rodapé, QR, ATCUD e métodos de pagamento.
        if frontend and isinstance(frontend, dict):
            payload['odoo_receipt'] = frontend
            for source, target in (
                ('qr_code', 'qr_code'), ('qrCode', 'qr_code'),
                ('atcud', 'atcud'), ('ATCUD', 'atcud'),
                ('hash_code', 'hash_code'), ('inalterable_hash', 'hash_code'),
            ):
                if frontend.get(source):
                    payload[target] = frontend[source]
        return payload
    ...
    def action_send(self):
        ...
        for job in self:
            if job.job_type == 'kitchen':
                # O ticket de bebidas só deve sair quando a mesa já estiver
                # atribuída. Enquanto não existir mesa, o job fica pendente e
                # o cron volta a tentar sem gastar tentativas.
                job.pos_order_id.invalidate_recordset()
                table = job._table_label(job.pos_order_id)
                if not table:
                    job.write({
                        'state': 'pending',
                        'last_error': 'A aguardar atribuição da mesa.',
                    })
                    continue
                refreshed = job._build_beverages_payload(job.pos_order_id)
                refreshed['job_id'] = job.name
                job.payload = json.dumps(refreshed, default=str)
