...
class PosPrintJob(models.Model):
    ...
    def _build_kitchen_payload(self, order, extra_payload=None):
        ...
        do pedido, a mesa e a lista de produtos com quantidade — sem
        preços, sem impostos, sem dados fiscais."""
        payload = {
            'job_id': None,
            ...
            'table': self._get_table_label(order) if order else False,
    ...
    @api.model
    def _get_table_label(self, order):
        """Devolve o nome da mesa (com o piso, se existir) associada à
        encomenda, usando o campo `table_id` do módulo de Restaurante
        do Odoo (pos_restaurant). Se o módulo não estiver instalado ou
        a encomenda não tiver mesa associada, devolve False — nesse
        caso o talão da cozinha simplesmente não mostra a mesa."""
        if not order or 'table_id' not in order._fields or not order.table_id:
            return False
        table = order.table_id
        floor_name = table.floor_id.name if table.floor_id else ''
        return ('%s - %s' % (floor_name, table.name)) if floor_name else table.name
